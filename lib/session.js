...
function Session(is_live, device_name, options) {
    ...
    var packet_ready = (header, packet) => {
        this.on_packet_ready(header, packet);
    };
    ...
    if (is_live) {
        ...
        this.readWatcher.callback = () => {
            var packets_read = this.session.dispatch();
            if (packets_read < 1) {
                this.empty_reads += 1;
            }
        };
...
Session.prototype.on_packet_ready = function (header, data) {
    var full_packet = new PacketWithHeader(data, header, this.link_type);
